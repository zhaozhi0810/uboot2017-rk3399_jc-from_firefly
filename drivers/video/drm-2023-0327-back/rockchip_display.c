...
//	printf("111.gd->lcd_type = %d\n",gd->lcd_type);
	native_mode = ofnode_find_subnode(timing, "timing");
	if (!ofnode_valid(native_mode)) {
		printf("display_get_timing_from_dts gd->lcd_type = %d\n",gd->lcd_type);
		if(gd->lcd_type >= 0)
		{
			lcd_type = gd->lcd_type & 0x3;   //只判断低两位
			if(lcd_type == 2)	//捷诚第二块5寸屏			
				phandle = ofnode_read_u32_default(timing, "native-mode-5inch-2", -1);  //2022-12-09
			else
				phandle = ofnode_read_u32_default(timing, lcd_type?"native-mode-7inch":"native-mode-5inch", -1);  //2022-12-05
		}
		else //默认为接口板的时序
